/**
 * lib/skill.ts — loads the universal skills + firm rulebook from DEMO DATA.
 *
 ...
 * effectively cached, no runtime document parsing. The teammate edits the files
 * (or drops new universal skills into DEMO DATA) and the app picks them up on
 * next boot.
 *
...
export const SKILL_MD = stripFrontmatter(
  read(SKILL_FILE, DEMO_DIR, process.cwd()),
);
export const CHECKLIST_MD = read(CHECKLIST_FILE, DEMO_DIR).trim();
export const PLAYBOOK_MD = read(PLAYBOOK_FILE, DEMO_DIR).trim();

/**
 * Any *other* .md files dropped into DEMO DATA (future universal skills for
 * evaluation/grading/questioning) are appended automatically.
 */
function extraSkills(): string {
  let dir: string[];
  try {
    dir = fs.readdirSync(DEMO_DIR);
  } catch {
    return "";
  }
  ...
  return dir
    ...
    .map((f) => stripFrontmatter(read(f, DEMO_DIR)))
    .filter(Boolean)
    .join("\n\n");
}
